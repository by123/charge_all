...
public class TreeViewAdapter<T> extends RecyclerView.Adapter {
    ...
    TreeViewAdapter(Context context, TreeNode<T> root,
    ...
    private void insertNode(List<TreeNode> nodeList, TreeNode<T> treeNode) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int level) {
        View view = LayoutInflater.from(context).inflate(baseNodeViewFactory.getLayoutId(level), parent, false);
    ...
    private void setupCheckableItem(View nodeView,
        ...
        if (view instanceof Button) {
            final Button checkableView = (Button) view;

            if (treeNode.isPartialSelected()) {
                checkableView.setBackgroundResource(R.drawable.ic_whitelist_partial_selected);
            } else {
                if (treeNode.isSelected()) {
                    checkableView.setBackgroundResource(R.drawable.ic_whitelist_item_checked);
                } else {
                    checkableView.setBackgroundResource(R.drawable.ic_whitelist_item_unchecked);
                }
            }
...
//                    checkableView.setButtonDrawable(null);
//                    boolean checked = checkableView.isChecked();
                    boolean checked = !treeNode.isSelected();
                    if (checked) {
                        checkableView.setBackgroundResource(R.drawable.ic_whitelist_item_checked);
                    } else {
                        checkableView.setBackgroundResource(R.drawable.ic_whitelist_item_unchecked);
    ...
    void deleteNode(TreeNode<T> node) {
        ...
        List<TreeNode<T>> allNodes = TreeHelper.getAllNodes(root);
