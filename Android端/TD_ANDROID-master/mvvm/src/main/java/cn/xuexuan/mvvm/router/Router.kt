package cn.xuexuan.mvvm.router

import android.app.Activity
import android.content.Intent
import android.os.Bundle
import android.os.Parcelable
import androidx.annotation.Nullable
import androidx.core.app.ActivityCompat
import androidx.core.app.ActivityOptionsCompat
import cn.xuexuan.mvvm.AppConstants
import java.io.Serializable
import java.util.*

/**
 * Created by xuexuan on 2016/11/29.
 */
class Router private constructor() {

    private val intent: Intent?
    private var from: Activity? = null
    private var to: Class<*>? = null
    private var data: Bundle? = null
    private var options: ActivityOptionsCompat? = null
    private var requestCode = -1
    private var enterAnim = AppConstants.ROUTER_ANIM_ENTER
    private var exitAnim = AppConstants.ROUTER_ANIM_EXIT

    private val bundleData: Bundle
        get() {
            if (data == null) {
                data = Bundle()
            }
            return data!!
        }

    init {
        intent = Intent()
    }

    fun to(to: Class<*>): Router {
        this.to = to
        return this
    }

    fun addFlags(flags: Int): Router {
        intent?.addFlags(flags)
        return this
    }

    fun data(data: Bundle): Router {
        this.data = data
        return this
    }

    fun putByte(@Nullable key: String, value: Byte): Router {
        bundleData.putByte(key, value)
        return this
    }

    fun putChar(@Nullable key: String, value: Char): Router {
        bundleData.putChar(key, value)
        return this
    }

    fun putInt(@Nullable key: String, value: Int): Router {
        bundleData.putInt(key, value)
        return this
    }

    fun putString(@Nullable key: String, value: String): Router {
        bundleData.putString(key, value)
        return this
    }

    fun putShort(@Nullable key: String, value: Short): Router {
        bundleData.putShort(key, value)
        return this
    }

    fun putFloat(@Nullable key: String, value: Float): Router {
        bundleData.putFloat(key, value)
        return this
    }

    fun putCharSequence(@Nullable key: String, @Nullable value: CharSequence): Router {
        bundleData.putCharSequence(key, value)
        return this
    }

    fun putParcelable(@Nullable key: String, @Nullable value: Parcelable): Router {
        bundleData.putParcelable(key, value)
        return this
    }

    fun putParcelableArray(@Nullable key: String, @Nullable value: Array<Parcelable>): Router {
        bundleData.putParcelableArray(key, value)
        return this
    }

    fun putParcelableArrayList(
        @Nullable key: String,
        @Nullable value: ArrayList<out Parcelable>
    ): Router {
        bundleData.putParcelableArrayList(key, value)
        return this
    }


    fun putIntegerArrayList(@Nullable key: String, @Nullable value: ArrayList<Int>): Router {
        bundleData.putIntegerArrayList(key, value)
        return this
    }

    fun putStringArrayList(@Nullable key: String, @Nullable value: ArrayList<String>): Router {
        bundleData.putStringArrayList(key, value)
        return this
    }

    fun putCharSequenceArrayList(
        @Nullable key: String,
        @Nullable value: ArrayList<CharSequence>
    ): Router {
        bundleData.putCharSequenceArrayList(key, value)
        return this
    }

    fun putSerializable(@Nullable key: String, @Nullable value: Serializable): Router {
        bundleData.putSerializable(key, value)
        return this
    }


    fun options(options: ActivityOptionsCompat): Router {
        this.options = options
        return this
    }

    fun requestCode(requestCode: Int): Router {
        this.requestCode = requestCode
        return this
    }

    fun anim(enterAnim: Int, exitAnim: Int): Router {
        this.enterAnim = enterAnim
        this.exitAnim = exitAnim
        return this
    }

    fun launch() {
        try {
            if (intent != null && from != null && to != null) {

                if (callback != null) {
                    callback!!.onBefore(from!!, to!!)
                }

                intent.setClass(from!!, to!!)

                intent.putExtras(bundleData)

                if (options == null) {
                    if (requestCode < 0) {
                        from!!.startActivity(intent)
                    } else {
                        from!!.startActivityForResult(intent, requestCode)
                    }

                    if (enterAnim > 0 && exitAnim > 0) {
                        from!!.overridePendingTransition(enterAnim, exitAnim)
                    }
                } else {
                    if (requestCode < 0) {
                        ActivityCompat.startActivity(from!!, intent, options!!.toBundle())
                    } else {
                        ActivityCompat.startActivityForResult(from!!, intent, requestCode, options!!.toBundle())
                    }
                }

                if (callback != null) {
                    callback!!.onNext(from!!, to!!)
                }
            }
        } catch (throwable: Throwable) {
            if (callback != null) {
                callback!!.onError(from!!, to!!, throwable)
            }
        }

    }

    companion object {

        val RES_NONE = -1

        private var callback: RouterCallback? = null

        fun newIntent(context: Activity): Router {
            val router = Router()
            router.from = context
            return router
        }

        fun pop(activity: Activity) {
            activity.finish()
        }

        fun setCallback(callback: RouterCallback) {
            Router.callback = callback
        }
    }
}
