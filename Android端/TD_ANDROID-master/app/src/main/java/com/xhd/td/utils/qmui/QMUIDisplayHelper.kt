...
object QMUIDisplayHelper {
    ...
    fun getScreenHeight(context: Context): Int {
        ...
        if (QMUIDeviceHelper.isXiaomi && xiaomiNavigationGestureEnabled(
                context
            )
        ) {
    ...
    fun getUsefulScreenWidth(activity: Activity): Int {
        return getUsefulScreenWidth(
            activity,
            QMUINotchHelper.hasNotch(activity)
        )
    }
    ...
    fun getUsefulScreenWidth(context: Context, hasNotch: Boolean): Int {
        ...
        if (isLandscape) {
            ...
            if (QMUIDeviceHelper.isHuawei && !huaweiIsNotchSetToShowInSetting(
                    context
                )
            ) {
    ...
    fun hasNavigationBar(context: Context): Boolean {
        ...
        return if (QMUIDeviceHelper.isVivo) {
    ...
    fun vivoNavigationGestureEnabled(context: Context): Boolean {
        val `val` = Settings.Secure.getInt(context.contentResolver,
            VIVO_NAVIGATION_GESTURE, 0)
        return `val` != 0
    }


    fun xiaomiNavigationGestureEnabled(context: Context): Boolean {
        val `val` = Settings.Global.getInt(context.contentResolver,
            XIAOMI_FULLSCREEN_GESTURE, 0)
        return `val` != 0
    }
    ...
    fun huaweiIsNotchSetToShowInSetting(context: Context): Boolean {
        // 0: 默认
        // 1: 隐藏显示区域
        val result = Settings.Secure.getInt(context.contentResolver,
            HUAWAI_DISPLAY_NOTCH_STATUS, 0)
        return result == 0
    }
    ...
    fun xiaomiIsNotchSetToShowInSetting(context: Context): Boolean {
        return Settings.Global.getInt(context.contentResolver,
            XIAOMI_DISPLAY_NOTCH_STATUS, 0) == 0
    }
}
