...
class OrderDetail extends React.Component {
  ...
  changePwdBook = (detail) => {
    this.props.dispatch(orderActions.changePwdBook({ deviceSn: detail.deviceSn }, () => {
      this.queryDetail(detail.orderId);
      message.success('修改密码本成功');
    }));
  }

  undoPwdChange = (detail) => {
    this.props.dispatch(orderActions.undoPwdChange({ deviceSn: detail.deviceSn }, () => {
      this.queryDetail(detail.orderId);
  ...
  render() {
    const {
      ...
      isSuperAdmin,
      changePwdBookResult: {
        loading: changePwdLoading = false,
      } = {},
    } = this.props;
    ...
    const deviceInfo = addAgentList(otherInfo); // 设备信息
    ...
    const isPwdBookChanged = otherInfo.pwdListChanged === 1;
    const changePwdBookBtn = (<div style={{ textAlign: 'center' }}>
      <Button loading={changePwdLoading} onClick={() => this.changePwdBook(incomeInfo)} disabled={isPwdBookChanged}>密码本修改</Button>
      <Button loading={changePwdLoading} style={{ marginLeft: 10 }} onClick={() => this.undoPwdChange(incomeInfo)} disabled={!isPwdBookChanged}>撤销</Button>
    </div>);
    let incomeColumns = [
    ...
    if (isSuperAdmin) {
      ...
      incomeColumns.push({
        ...
        render: text => (text === 1 ? changePwdBookBtn : ''),
      });
    }
    const profitColumns = [
      { key: 'servicePriceYuan', label: '消费金额', render: (text, record) => formatDecimal(text - record.refundMoneyYuan - record.refundingMoneyYuan) },
    ...
    const deviceColumns = [
      ...
      { key: 'deviceVersion', label: '设备版本', render: text => (text ? `V${text}` : '') },
      { key: 'deviceSn', label: '设备编号' },
      { key: 'deviceLocation', label: '所属区域' },
      { key: 'mchName', label: '所属商户' },
      {
        key: 'priceRule',
        label: '计费规则',
        render: (rule) => {
...
OrderDetail.propTypes = {
  ...
  changePwdBookResult: PropTypes.object,
...
const Container = connect(({ order: { orderDetail, changePwdBookResult }, global: { profile } }) => ({
