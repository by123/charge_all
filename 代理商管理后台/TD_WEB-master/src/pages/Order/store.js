...
function fetchOrderList(data) {
  ...
  return {
    type: types.fetchOrderList,
    payload: post(api.orderListPost, Object.assign({ pageSize: 15 }, data)),
  };
}

function recoverOrder(data) {
  return {
    type: types.recoverOrder,
    payload: post(api.recoverOrder, data),
...
function changePwdBook(data, nextAction) {
  return {
    type: types.changePwdBook,
    payload: post(api.changePwdBook, data),
    nextAction,
  };
}

function undoPwdChange(data, nextAction) {
  return {
    type: types.undoPwdChange,
    payload: post(api.undoPwdChange, data),
    nextAction,
  };
}
...
export const reducer = createReducer({
  ...
  [types.fetchOrderList](state, { payload }) {
    mapRowKey(payload);
  ...
  [types.recoverOrder](state, { payload }) {
    return mergeState(state, payload, 'recoveData');
  },
  ...
  [types.changePwdBook](state, { payload }) {
    return mergeState(state, payload, 'changePwdBookResult');
  },
});
