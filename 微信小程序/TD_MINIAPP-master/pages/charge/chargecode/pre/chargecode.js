...
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      hiddenFinish: options.showFinish == 0,
    ...
    if (data.chargingFlag == 1) {
      ...
      if (data.password != null) {
        ...
        this.setData({
          ...
          servicePriceRule:  JSON.parse(data.servicePriceRule)
        });
        //开始倒计时
        this.countDown();
  ...
  countDown: function () {
    ...
    this.data.timer = setInterval(function () {
      ...
      var userTime = 24 * 3600 - that.data.countDownNum
      var h = parseInt(userTime / 3600);
      var m = parseInt((userTime - h * 3600) / 60);
      var s = userTime - h * 3600 - m * 60;
      that.setData({
      ...
      success: function (res) {
        if (res.cancel) {
          ...
          netutil.getRequest(constant.URL_CLOSE_ORDER, params, function (success) {
            ...
            getApp().reLunchPage(chargeCompeletePage, [{ 'key': 'orderId', 'value': that.data.orderId}]);
          }, function (fail) { });
        }
      }
    })
