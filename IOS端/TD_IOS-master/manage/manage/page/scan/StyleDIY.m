...
+ (LBXScanViewStyle*)xhcStyle
{
    ...
    if(IS_IPHONE_678 || IS_IPHONE_678P){
        style.centerUpOffset = STWidth(47);
    }else{
        style.centerUpOffset = STWidth(87);
    }
    ...
    //四个角的颜色
    style.colorAngle = cwhite;
    //扫码框周围4个角的宽度
    style.photoframeAngleW = STWidth(16);
    
    //扫码框周围4个角的高度
    style.photoframeAngleH = STWidth(16);
    
    //扫码框内 动画类型 --线条上下移动
    style.anmiationStyle = LBXScanViewAnimationStyle_LineMove;
    
    //线条上下移动图片
    style.animationImage = [UIImage imageNamed:IMAGE_SCAN_LINE];
    
    style.notRecoginitonArea = c09;
    
    style.xScanRetangleOffset = STWidth(68);
