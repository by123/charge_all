...
@interface MerchantDetailView()<UITableViewDelegate,UITableViewDataSource>

@property(strong, nonatomic)AgentDetailViewModel *mViewModel;
@property(strong, nonatomic)UITableView *infoTableView;
@property(strong, nonatomic)UITableView *ruleTableView;
...
-(void)initView{
    ...
    _agentTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, STHeight(44*2) + tableHeight, ScreenWidth, _mViewModel.agentDatas.count* STHeight(50))];
    ...
    NSString *ruleStr = @"计费规则";
    UILabel *ruleLabel = [[UILabel alloc]initWithFont:STFont(14) text:ruleStr textAlignment:NSTextAlignmentLeft textColor:c10 backgroundColor:nil multiLine:NO];
    CGSize ruleSize = [ruleStr sizeWithMaxWidth:ScreenWidth font:STFont(14)];
    ruleLabel.frame = CGRectMake(STWidth(15), STHeight(20) + STHeight(44*2) + tableHeight, ruleSize.width, STHeight(20));
    [_scrollView addSubview:ruleLabel];
    
    
    _ruleTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, STHeight(44*3) + tableHeight, ScreenWidth, _mViewModel.ruleDatas.count* STHeight(50))];
    _ruleTableView.delegate = self;
    _ruleTableView.dataSource = self;
    [_ruleTableView setScrollEnabled:NO];
    [_ruleTableView useDefaultProperty];
    
    [_scrollView addSubview:_ruleTableView];
    
    tableHeight += _mViewModel.ruleDatas.count* STHeight(50);
    ...
    deviceLabel.frame = CGRectMake(STWidth(15), STHeight(20) + STHeight(44*3) + tableHeight, deviceSize.width, STHeight(20));
    [_scrollView addSubview:deviceLabel];
    
    
    _deviceTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, STHeight(44*4) + tableHeight, ScreenWidth, _mViewModel.deviceDatas.count* STHeight(50))];
    ...
    addressLabel.frame = CGRectMake(STWidth(15), STHeight(20) + STHeight(44*4) + tableHeight , addressSize.width, STHeight(20));
    ...
    [_scrollView setContentSize:CGSizeMake(ScreenWidth, STHeight(44 * 5)+ tableHeight + STHeight(77) + STHeight(343))];
    ...
    //出租车隐藏编辑
    if(_mViewModel.model.mchType == 1 && _mViewModel.model.level == 2){
        _editBtn.hidden = YES;
    }
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    }else if(tableView == _ruleTableView){
        return [_mViewModel.ruleDatas count];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == _infoTableView){
        ...
        if(indexPath.row == [_mViewModel.ruleDatas count] - 1){
            [cell updateData:[_mViewModel.ruleDatas objectAtIndex:indexPath.row] lineHidden:YES];
        }else{
            [cell updateData:[_mViewModel.ruleDatas objectAtIndex:indexPath.row] lineHidden:NO];
        }
    }
    else{
...
-(void)onEditBtnClick{
    if(_mViewModel){
        [_mViewModel goMerchantEditPage];
    }
}
...
-(void)initAddressView{
    CGFloat tableHeight = (_mViewModel.infoDatas.count + _mViewModel.deviceDatas.count + _mViewModel.ruleDatas.count + _mViewModel.agentDatas.count)* STHeight(50);
    if(IS_NS_COLLECTION_EMPTY(_mViewModel.agentDatas)){
        tableHeight -= STHeight(44);
    }
    
    _addressView = [[UIView alloc]initWithFrame:CGRectMake(0, STHeight(44) * 5 + tableHeight, ScreenWidth, STHeight(343))];
    ...
    if(_mViewModel.model.mchLocationLongitude == 0 ||
       _mViewModel.model.mchLocationLatitude == 0){
        ...
        _addressView.frame = CGRectMake(0, STHeight(44) * 5 + tableHeight, ScreenWidth, STHeight(72));
    }else{
