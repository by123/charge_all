...
@implementation OrderCompeleCell
...
-(void)initView{
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(STWidth(15), 0, STWidth(345), STHeight(357))];
    ...
    _refundBtn = [[UIButton alloc]initWithFont:STFont(16) text:@"退款" textColor:c10 backgroundColor:cwhite corner:2 borderWidth:1 borderColor:c10];
    [_refundBtn setBackgroundColor:c03 forState:UIControlStateHighlighted];
    _refundBtn.frame = CGRectMake(STWidth(240), STHeight(306), STWidth(90), STHeight(36));
    [view addSubview:_refundBtn];
...
-(void)updateData:(OrderModel *)model position:(NSInteger)position{
    ...
    NSString *statuStr;
    if(model.orderStateWeb == OrderType_Paid){
        statuStr =@"已支付";
    ...
    if(model.canRefund){
        [_refundBtn addTarget:self action:@selector(onClickRefund:) forControlEvents:UIControlEventTouchUpInside];
        _refundBtn.tag = position;
    }
    _refundBtn.hidden = !model.canRefund;

    
}

-(void)onClickRefund:(id)sender{
    UIView *view = sender;
    if(_delegate){
        [_delegate OnClickRefundBtn:view.tag];
    }
}

+(NSString *)identify{
    return NSStringFromClass([OrderCompeleCell class]);
}
